...
| R2/R3 workflow-contract semantics (compensation/replay/HITL) | Deployed route runtime is parse -> fetch -> generate -> evaluate with no compensation/HITL branches | Misrepresented | Process-scope governance is not validated in this PoC |
| HITL mandatory/non-waivable for high-risk paths | No HITL gate exists in the deployed route runtime path | Misrepresented | Critical control behavior is untested |
