...
export class FlutterAgentCorePocStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const nativeLambda = makeLambda('RunNativeToolFn', 'fetch_native_stage.handler');
    const mcpLambda = makeLambda('RunMcpToolFn', 'fetch_mcp_stage.handler');
    const generateLambda = makeLambda('GenerateResponseFn', 'generate_stage.handler');
    const evaluateLambda = makeLambda('EvaluateRunFn', 'evaluate_stage.handler');
    const jiraToolLambda = makeLambda('JiraToolTargetFn', 'jira_tool_target.handler');
    ...
    const runtime = new agentcore.Runtime(this, 'SopAgentRuntime', {
      ...
      description: 'SOP runtime for native vs MCP Jira orchestration comparison',
    ...
    const gateway = new agentcore.Gateway(this, 'SopGateway', {
      ...
      protocolConfiguration: new agentcore.McpProtocolConfiguration({
        instructions: 'Use Jira issue tools for case triage and customer response SOP tasks',
      ...
      toolSchema: agentcore.ToolSchema.fromInline([
        ...
        },
        {
          ...
          inputSchema: {
            type: agentcore.SchemaDefinitionType.OBJECT,
            properties: {
              issue_key: { type: agentcore.SchemaDefinitionType.STRING, description: 'Issue key such as JRASERVER-79286' },
            },
            required: ['issue_key'],
          },
          outputSchema: {
        ...
        },
        {
          ...
          outputSchema: {
            ...
            properties: {
              key: { type: agentcore.SchemaDefinitionType.STRING },
              labels: { type: agentcore.SchemaDefinitionType.ARRAY, items: { type: agentcore.SchemaDefinitionType.STRING } },
            },
            required: ['key', 'labels'],
          },
        },
        ...
        },
        {
          ...
          outputSchema: {
            ...
            properties: {
              key: { type: agentcore.SchemaDefinitionType.STRING },
              risk_flags: { type: agentcore.SchemaDefinitionType.ARRAY, items: { type: agentcore.SchemaDefinitionType.STRING } },
            },
    ...
    const nativeGenerateTask = new sfnTasks.LambdaInvoke(this, 'GenerateCustomerResponseNativeStage', {
      lambdaFunction: generateLambda,
      payloadResponseOnly: true,
    });

    const nativeEvaluateTask = new sfnTasks.LambdaInvoke(this, 'EvaluateExecutionNativeStage', {
      lambdaFunction: evaluateLambda,
      payloadResponseOnly: true,
    });

    const mcpGenerateTask = new sfnTasks.LambdaInvoke(this, 'GenerateCustomerResponseMcpStage', {
      lambdaFunction: generateLambda,
      payloadResponseOnly: true,
    });

    const mcpEvaluateTask = new sfnTasks.LambdaInvoke(this, 'EvaluateExecutionMcpStage', {
      lambdaFunction: evaluateLambda,
      payloadResponseOnly: true,
    });

    const nativeChain = nativeTask.next(nativeGenerateTask).next(nativeEvaluateTask);
      ...
      targets: [
        new targets.SfnStateMachine(stateMachine, {
          input: events.RuleTargetInput.fromObject({
            flow: 'mcp',
            request_text: 'Please triage JRASERVER-79286 and draft a customer-safe response update.',
    ...
    new CfnOutput(this, 'RuntimeEndpointArn', { value: runtimeEndpoint.agentRuntimeEndpointArn });
    ...
    new CfnOutput(this, 'ArtifactsBucketName', { value: datasetBucket.bucketName });
  }
}
