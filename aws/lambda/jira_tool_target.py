...
def _extract_issue_key(event: Dict[str, Any], arguments: Dict[str, Any]) -> str:
    if arguments.get("issue_key"):
        return str(arguments["issue_key"])
...
def _result_issue_by_key(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    return issue


def _result_issue_status_snapshot(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    return {"key": issue["key"], "status": issue.get("status", "Unknown"), "updated": issue.get("updated", "")}


def _result_issue_priority_context(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
...
def _result_issue_labels(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    return {"key": issue["key"], "labels": issue.get("labels", [])}


def _result_issue_project_key(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    key = issue.get("key", "")
    project_key = key.split("-", 1)[0] if "-" in key else ""
    return {"key": key, "project_key": project_key}


def _result_issue_update_timestamp(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    return {"key": issue["key"], "updated": issue.get("updated", "")}


def _result_issue_risk_flags(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
...
def _result_customer_sentiment(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    return {"key": issue["key"], "sentiment": _derive_sentiment(issue), "status": issue.get("status", "Unknown")}


def _result_customer_message_seed(issue: Dict[str, Any], _arguments: Dict[str, Any]) -> Dict[str, Any]:
    summary = str(issue.get("summary", "")).strip()
    return {"key": issue["key"], "seed_message": summary[:180]}


def _result_write_issue_followup_note(issue: Dict[str, Any], arguments: Dict[str, Any]) -> Dict[str, Any]:
    note_text = str(arguments.get("note_text", "")).strip()
...
_TOOL_RESULT_BUILDERS: Dict[str, Callable[[Dict[str, Any], Dict[str, Any]], Dict[str, Any]]] = {
    ...
    "jira_write_issue_followup_note": _result_write_issue_followup_note,
...
def handler(event: Dict[str, Any], _context: Any) -> Dict[str, Any]:
    ...
    result = _build_tool_result(tool_name=tool_name, issue=issue, arguments=arguments)
