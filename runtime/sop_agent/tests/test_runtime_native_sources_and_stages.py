...
from runtime.sop_agent.sources import mcp_source, native_source
from runtime.sop_agent.stages import evaluate_stage, fetch_mcp_stage, fetch_native_stage, generate_stage, parse_stage


def test_execute_native_source_delegates_to_runtime_fetch_native_stage(monkeypatch: pytest.MonkeyPatch) -> None:
    event: Dict[str, Any] = {"request_text": "native"}

    def _handler(received_event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        assert received_event is event
        assert context is None
        return {"delegated": "native"}

    monkeypatch.setattr(native_source, "fetch_native_stage_handler", _handler)
    assert native_source.execute_native_source(event) == {"delegated": "native"}


def test_execute_mcp_source_delegates_to_runtime_fetch_mcp_stage(monkeypatch: pytest.MonkeyPatch) -> None:
    event: Dict[str, Any] = {"request_text": "mcp"}

    def _handler(received_event: Dict[str, Any], context: Any) -> Dict[str, Any]:
        assert received_event is event
        assert context is None
        return {"delegated": "mcp"}

    monkeypatch.setattr(mcp_source, "fetch_mcp_stage_handler", _handler)
    assert mcp_source.execute_mcp_source(event) == {"delegated": "mcp"}
...
def test_fetch_native_tool_catalog() -> None:
...
def test_fetch_native_tool_invocations(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        fetch_native_stage,
        "fetch_jira_issue",
        lambda **_kwargs: {
    ...
    cases = [
        ("jira_api_get_issue_by_key", "JRASERVER-1", "r", "key", "JRASERVER-1"),
        ("jira_api_get_issue_status_snapshot", "JRASERVER-1", "r", "status", "Done"),
        ("jira_api_get_issue_priority_context", "JRASERVER-1", "r", "risk_band", "high"),
        ("jira_api_get_issue_labels", "JRASERVER-1", "r", "labels", ["x"]),
        ("jira_api_get_issue_project_key", "ABC-1", "r", "project_key", "JRASERVER"),
        ("jira_api_get_issue_update_timestamp", "JRASERVER-1", "r", "updated", "t"),
        (
    ...
    for tool_name, issue_key, request_text, field, expected in cases:
        result = fetch_native_stage._invoke_native_tool(
            tool_name,
            issue_key,
            "https://jira",
...
def test_fetch_native_tool_invocation_invalid_tool(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        fetch_native_stage,
        "fetch_jira_issue",
        lambda **_kwargs: {"key": "JRASERVER-1"},
    )
    with pytest.raises(RuntimeError):
